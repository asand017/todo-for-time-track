...
export default function Task(props) {
  ...
  const [startTime, setStartTime] = useState(
    DateTime.fromFormat(props.start, "TT").toLocaleString(DateTime.TIME_SIMPLE)
  );
  const [endTime, setEndTime] = useState(
    DateTime.fromFormat(props.end, "TT").toLocaleString(DateTime.TIME_SIMPLE)
  );
  ...
  const [errorStatus, setErrorStatus] = useState(
    isBefore(parseISO(props.day), subDays(Date.now(), 1)) ? true : false
  );
  ...
  useEffect(() => {
    setStartTime(
      DateTime.fromFormat(props.start, "TT").toLocaleString(
  ...
  return (
    <>
      ...
      <InfoDialog
        ...
        day={
          props.day
            ? DateTime.fromISO(props.day).toLocaleString(DateTime.DATE_SHORT)
            : null
        }
    ...
    <>
      ...
      >
        <DialogTitle>
          ...
          <div
            className="priority"
            style={{ color: priorityColors[props.task.priority] }}
          >
            Priority: {props.task.priority}
          </div>
          <div className="date-time">
            <div style={{ color: props.dayStatus ? "red" : "none" }}>
              {props.day}
            </div>
